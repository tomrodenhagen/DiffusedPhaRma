...
build_model = function(equation, drift, diffusion, params, prior = list(), bounds = list(),
    observation_equation = ConcObserved ~ Conc, transf = identity, n_retrys = 10) {
    model <- ctsm$new()
    ...
    myModel = list(cstmModel = model, drift = drift, diffusion = diffusion, estimate = estimate,
        simulate = simulate, equation = equation, params = params, prior = prior,
        bounds = bounds, transf = transf, observation_equation = observation_equation,
        n_retrys = n_retrys)
    return(myModel)
}
copy_model = function(model) {
    return(build_model(model$equation, model$drift, model$diffusion, model$params,
        model$prior, model$bounds, model$observation_equation, model$transf, model$n_retrys))
}

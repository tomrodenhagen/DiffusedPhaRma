...
init_run_folder = function(experiment_folder, run_number, config) {
    ...
    write_json(info_json, file.path(folder_path, "info.json"))
...
prepare_run_folder = function(experiment_folder, config) {
    ...
    if (config$fresh | is.null(last_run_folder)) {
        working_folder = init_run_folder(experiment_folder, get_run_number(last_run_folder) +
            1, config)
    } else {
...
get_conf_string = function(point, lower, upper, test = NULL, full = FALSE) {
    if (full) {
        s = paste(round(point, 2), "(", round(lower, 2), ",", round(upper, 2), ")",
            sep = "")
    } else {
...
rename_shortcut = function(shortcut, invert = FALSE) {
    renaming = list(const_diff = "$\\sigma_\\tau dW$", linear_diff = "$C \\cdot \\sigma_\\tau dW$",
        const_diff_no_retry = "$\\sigma_\\tau dW$, kein Multistart", linear_diff_no_retry = "$C \\cdot \\sigma_\\tau dW$, kein Multistart",
        low_noise = "$\\sigma_\\epsilon^2 = 0.01$", medium_noise = "$\\sigma_\\epsilon^2 = 0.05$",
        high_noise = "$\\sigma_\\epsilon^2 = 0.1$", const_diff_fixed_low_noise = "$\\sigma_\\tau dW$,Fix. $\\sigma_\\epsilon^2 = 0.01$",
        ...
        measure_first_cycle = "$k=1, n=6$", measure_3th_cycle = "$k=4, n = 6$", measure_6th_cylce = "$k=7, n= 6$",
        measure_first_cycle_few_samples = "$k=1, n=3$", measure_first_cycle_many_samples = "$k=1, n=11$")
    if (invert) {
...
plot_Km = function(dfs, path, name) {
    models = lapply(dfs, function(x) {
        x[["Modell"]]
    })
    designs = lapply(dfs, function(x) {
        x[["Design"]]
    })
    parameters = lapply(dfs, function(x) {
        x[["Parameter"]]
    })
    for (m in unique(models)) {
        for (p in parameters) {
            ...
            if (length(df_filtered) > 1) {
                plot_dfs(df_filtered, file.path(path, paste(name, "unfixed", rename_shortcut(m,
                  invert = TRUE), rename_shortcut(p, invert = TRUE), "per_km.png",
                  sep = "_")), "Design", m, p)
            }
        }
    }
    for (m in unique(models)) {
        for (p in designs) {
            ...
            if (length(df_filtered) > 1) {
                plot_dfs(df_filtered, file.path(path, paste(name, "fixed", rename_shortcut(m,
                  invert = TRUE), rename_shortcut(p, invert = TRUE), "per_km.png",
                  sep = "_")), "Parameter", m, p)
            }
        }
...
plot_dfs = function(dfs, path, unfixed, fixed1, fixed2) {
    png(path)
    plot(NULL, NULL, ylab = "Test Power", xlim = c(2, 10), ylim = c(0, 1.1), xlab = TeX("$K_m$"))
    cols = c()
    names = c()
    ...
    for (i in seq_along(dfs)) {
        ...
        polygon(c(df[["Km"]][, "group"], rev(df[["Km"]][, "group"])), c(df[["Km"]][,
            "Lower"], rev(df[["Km"]][, "Upper"])), col = col, density = 30)
        points(df[["Km"]][, "group"], df[["Km"]][, "Point"], pch = 19, col = col,
            cex = 1.5)
...
make_typ2_res_row = function(m, s, d, res, n_sim) {
    row = list(Modell = rename_shortcut(m$shortcut), Parameter = rename_shortcut(s$shortcut),
    ...
    for (i in rownames(km_table)) {
        if (km_table[i, "group"] %in% special_kms()) {
            ...
            row[[col_name]] = list(Point = km_table[i, "Point"], Lower = km_table[i,
                "Lower"], Upper = km_table[i, "Upper"], N_samples = km_table[i, "N_samples"])
        }
        row[["Nicht konv. Typ-II"]] = paste(round(res[["not_valid"]]/n_sim * 100,
            1), "\\%")
    }
    return(row)
}
        ...
        if (grepl("K_m", col, fixed = TRUE)) {
            for (row in rownames(df_fixed)) {
                ...
                for (row_other in rownames(df_other)) {
                  ...
                  df_typ2[row_other, col] = get_conf_string(km_other$Point, km_other$Lower,
                    km_other$Upper, test_res)
                  if (df_typ2[row_other, "Design"] == fixed_design & df_typ2[row_other,
                    "Modell"] == fixed_model) {
                    df_typ2[row_other, col] = paste("\\colorbox{blue!30}{", df_typ2[row_other,
                      col], "}", sep = "")
                  }
                }
            }
    ...
    for (col in colnames(df_typ2)) {
        if (grepl("K_m", col, fixed = TRUE)) {
            for (row in rownames(df_typ2)) {
                ...
                df_typ2[row, col] = get_conf_string(as.numeric(cell$Point), cell$Lower,
                  cell$Upper)
            }
        }
    }
...
print_aggregated_results = function(res, recent_res_folder, config) {
    ...
    for (design in unique(df_typ2$Design)) {
        df_list[[design]] = add_significance(df_typ2[df_typ2$Design == design, ],
            fixed_design = design)
    }
    ...
    print(xtable(df_typ2_design, type = "latex"), include.rownames = FALSE, floating = FALSE,
        file = file.path(recent_res_folder, paste(config$name, "res_typ2.tex", sep = "_")),
        sanitize.colnames.function = identity, sanitize.text.function = identity)
    print(xtable(df_typ2_model, type = "latex"), include.rownames = FALSE, floating = FALSE,
        file = file.path(recent_res_folder, paste(config$name, "res_typ2_model.tex",
            sep = "_")), sanitize.colnames.function = identity, sanitize.text.function = identity)
    print(xtable(df_typ1, type = "latex"), include.rownames = FALSE, floating = FALSE,
        file = file.path(recent_res_folder, paste(config$name, "res_typ1.tex", sep = "_")),
        sanitize.colnames.function = identity, sanitize.text.function = identity)
...
make_res_path = function(config) {
    ...
    return(file.path(recent_res_folder, paste(config$name, config$mode, "_res.RDS")))
}
...
get_aggregated_results = function(models, designs, parameter_samplings, working_folder,
    config) {
    res_agg_typ1 = list()
    res_agg_typ2 = list()
    res_agg_km = list()
    for (m in models) {
        for (s in parameter_samplings) {
            for (d in designs) {
                ...
                scenario = list(name = name, description = desc, model_H0 = m$H0,
                  model_H1 = m$H1, design = d$design, sample_params = s$sampling,
                  T_statistic = T_statistic, H0_drift = H0_drift, H1_drift = H1_drift)
                res = run_complete_scenario(scenario, working_folder, config$n_simulations_typ1,
                  config$n_simulations_typ2, config$n_samples, alpha = config$alpha,
                  h = config$h, config$fresh)
                eval_res_typ1 = eval_simulation(res$type1)
                eval_res_typ2 = eval_simulation(res$type2)
                res_agg_typ2[[name]] = make_typ2_res_row(m, s, d, eval_res_typ2,
                  config$n_simulations_typ2)
                res_agg_typ1[[name]] = list(Modell = rename_shortcut(m$shortcut),
                  Parameter = rename_shortcut(s$shortcut), Design = rename_shortcut(d$shortcut),
                  `Emp. Typ-I Fehler` = get_conf_string(eval_res_typ1$Point, eval_res_typ1$Lower,
                    eval_res_typ1$Upper), `Nicht konv.Typ-I` = paste(eval_res_typ1[["not_valid"]]/config$n_simulations_typ1 *
                    100, "\\%"))
                res_agg_km[[name]] = list(Modell = rename_shortcut(m$shortcut), Parameter = rename_shortcut(s$shortcut),
                  Design = rename_shortcut(d$shortcut), Km = eval_res_typ2[["per_km"]])
            }
        }
    }
...
run_scenarios = function(models, designs, parameter_samplings, config, final_res = FALSE) {
    ...
    res = get_aggregated_results(models, designs, parameter_samplings, working_folder,
        config)
    saveRDS(res, make_res_path(config))
    if (final_res) {
        print_aggregated_results(res, init_final_res_folder(config), config)
    } else {
        print_aggregated_results(res, recent_res_folder, config)
    }
}

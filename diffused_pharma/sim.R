...
get_t_grid = function(t_start, t_end, n_samples, h, list_of_dosis) {
    ...
    output_samples_df = data.frame(t = output_samples, output = rep(TRUE, length(output_samples)),
        D = rep(NA, length(output_samples)), D_sample = rep(FALSE, length(output_samples)))
    simulation_samples_df = data.frame(t = simulation_samples, output = rep(FALSE,
        length(simulation_samples)), D = rep(NA, length(simulation_samples)), D_sample = rep(FALSE,
        length(simulation_samples)))
    dosis_samples_df = data.frame(t = dosis_samples, output = rep(FALSE, length(dosis_samples)),
        D = rate, D_sample = rep(TRUE, length(dosis_samples)))
...
simulate_model <- function(drift, diffusion, params, t_start, t_end, n_samples = 20,
    h = 0.1, dosis = list()) {

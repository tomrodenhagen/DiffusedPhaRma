...
png(file="./plots/fixed_design_vis.png", width=5, height=5,
...
for(km in c(5))
{
  ...
  data_unobs = simulate_model(drift, diff, parameter_no_noise, design$t_start, design$t_end + 0.5 , n_samples = 300, dosis = design$dosis, h=0.01 )
  ...
  lines(data_obs_no_nan[["t"]], data_obs_no_nan[["ConcObserved"]],
       ylim=c(0, 15),
       ylab="Drug concentration in mg /l",
       xlab="time in days", pch=19,col="red",cex=1.5)
  
  
  lines(data_unobs[["t"]], data_unobs[["ConcObserved"]],
        ylab="Drug concentration in mg /l",
        xlab="time in days", pch=19,col="green",cex=1.5)
  legend( x= "topright", legend=c("Concentration", "Measurement", "Dosis"),
          col=c("green","blue", "red"), lty=1:2, cex=1,pch=19)

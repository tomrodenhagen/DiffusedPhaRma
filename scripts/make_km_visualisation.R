...
design = list(t_start = 0, t_end = n_days, n_samples = get_samples(n_days, 0, sampling = sampling),
    dosis = dosis)
png(file = "./plots/km_vis.png", width = 5, height = 5, units = "in", res = 300,
    pointsize = 8)
par(mfrow = c(2, 2))
for (km in c(3, 5, 7, 9)) {
    ...
    plot(data_obs[["t"]], data_obs[["ConcObserved"]], main = TeX(paste("K_m=", km,
        sep = "")), ylim = c(0, 15), ylab = "Drug concentration in mg /l", xlab = "time in days",

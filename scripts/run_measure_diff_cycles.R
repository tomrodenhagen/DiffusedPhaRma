...
design_measure_at_6th_dose = list(t_start = 0, t_end = n_days, n_samples = get_samples(n_days,
    6), dosis = dosis)
design_measure_at_3th_dose = list(t_start = 0, t_end = n_days, n_samples = get_samples(n_days,
    3), dosis = dosis)
...
parameter_samplings = list(list(sampling = get_parameter_sampling(low_noise), desc = "Uniform sampling of Km between 3 and 9,  CL=1.75, V=50 and sigma_eps is low variance",
    ...
    shortcut = "high_noise"))
designs = list(list(design = design_measure_at_3th_dose, desc = "Dosing at every day and measure shortly after and shortly before.
               Additionaly measure some many points after the third dose",
    shortcut = "measure_3th_cycle"), list(design = design_measure_at_6th_dose, desc = "Dosing at every day and measure shortly after and shortly before.
